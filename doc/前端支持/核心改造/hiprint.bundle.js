    ...
    jQuery, n = "connected", i = "reconnecting", window.hiwebSocket = {
        ...
        send: function send(t) {
            try {
                ...
                toastr.success("发送数据失败:" + (t || "") + JSON.stringify(e),'提醒')
            }
        },
        ...
        start: function start() {
            ...
            var t = this;
            //判断是否存在打印机信息
            var printInfo = getLocalStorageObj("pda_printer_info");
            if (printInfo == null) {
                return false;
            }
            window.WebSocket ? this.socket || (this.socket = io("http://"+ printInfo.deviceIp + ":17521", {
                reconnectionAttempts: 3
            }), this.socket.on("connect", function (e) {
                t.opened = !0, console.log("Websocket opened.");toastr.success('连接打印客户端成功', '提醒'),
                    _this.socket.on("successs", function (t) {
                    //  监听打印成功的返回消息
                    hinnn.event.trigger("printSuccess_" + t.templateId, t);
                    console.log(t.templateId);
                    console.log("打印成功");
                    toastr.success(t.msg, '提醒')
                }), _this.socket.on("error", function (t) {
                    // 监听打印失败异常消息
                    hinnn.event.trigger("printError_" + t.templateId, t);
                    console.log(t.templateId);
                    console.log(JSON.stringify(t))
                    toastr.error(t.msg, '提醒');
                }), _this.socket.on("printerList", function (e) {
                    //监听获取打印机列表消息
                    t.printerList = e;
                    // console.log("打印机列表："+ JSON.stringify(e));
                }), t.state = n;
    ...
    var _r,
        ...
        ct = function () {
            ...
            return t.prototype.design = function (t, e) {
                ...
                } else toastr.error('连接打印客户端失败', '提醒');
            }, t.prototype.imageToBase64 = function (t) {
                var e = $(t).attr("src");
